...
export class BlogsController {
    ...
    async create(@Body() blog: BlogDto, @Request() req): Promise<BlogEntity> {
        // create a new blog and return the newly created poblogst
        return await this.blogService.create(blog, req.user.id);
    }

    @UseGuards(AuthGuard('jwt'))
    @Put(':id')
    async update(@Param('id') id: number, @Body() blog: BlogDto, @Request() req): Promise<BlogEntity> {
        // get the number of row affected and the updated blog
        const { numberOfAffectedRows, updatedBlog } = await this.blogService.update(id, blog, req.user.id);
